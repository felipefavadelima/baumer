...
"""
rPPG ao vivo com interface moderna (PySide6 + pyqtgraph) + tracking facial.

Recursos principais
-------------------
- Captura ao vivo via câmera Baumer (neoapi) ou webcam OpenCV.
- Seleção de ROI diretamente sobre o vídeo com o mouse.
- Tracking facial híbrido: detector facial periódico + tracker leve entre detecções.
- ROI "travada" ao rosto: a ROI manual passa a se mover/escalar com a face.
- Exibição contínua do sinal rPPG usando o algoritmo POS.
- FFT/BPM atualizada periodicamente (padrão: a cada 10 s).
- ROI e caixa do rosto sempre desenhadas sobre a imagem.
...
Uso
---
python rppg_live_modern_gui_face_tracking.py
"""
...
DEFAULT_FACE_DETECT_EVERY_S = 0.8
DEFAULT_FACE_DETECT_WIDTH = 360
...
def rect_center_distance2(a: Tuple[int, int, int, int], b: Tuple[int, int, int, int]) -> float:
    ax, ay, aw, ah = a
    bx, by, bw, bh = b
    acx = ax + 0.5 * aw
    acy = ay + 0.5 * ah
...
def convert_to_tracking_bgr8(frame_bgr: np.ndarray) -> np.ndarray:
    ...
    max_val = int(np.max(frame_bgr)) if frame_bgr.size else 0
    if max_val <= 0:
    ...
    if max_val <= 4095:
        ...
        alpha = 255.0 / float(max_val)
    return cv2.convertScaleAbs(frame_bgr, alpha=alpha)
...
class FaceTracker:
    def __init__(self, detect_every_s: float = DEFAULT_FACE_DETECT_EVERY_S, detect_width: int = DEFAULT_FACE_DETECT_WIDTH):
        self.detect_every_s = float(max(0.2, detect_every_s))
        self.detect_width = int(max(160, detect_width))
        cascade_path = cv2.data.haarcascades + "haarcascade_frontalface_default.xml"
        self.face_cascade = cv2.CascadeClassifier(cascade_path)
        self.tracker = None
        self.tracker_kind = "None"
        self.face_box: Optional[Tuple[int, int, int, int]] = None
        self.tracked_roi: Optional[Tuple[int, int, int, int]] = None
        self.roi_rel_face: Optional[Tuple[float, float, float, float]] = None
        self.last_detect_time = 0.0
        self.pending_roi: Optional[Tuple[int, int, int, int]] = None
    ...
    def reset(self) -> None:
        ...
        self.roi_rel_face = None
        self.last_detect_time = 0.0
        self.pending_roi = None
        self.force_redetect = False

    def request_roi(self, roi: Optional[Tuple[int, int, int, int]]) -> None:
        self.pending_roi = roi
        self.force_redetect = True
        if roi is None:
            self.reset()

    def _detect_faces(self, frame_bgr8: np.ndarray) -> list[Tuple[int, int, int, int]]:
        if self.face_cascade.empty():
            return []
        gray = cv2.cvtColor(frame_bgr8, cv2.COLOR_BGR2GRAY)
        gray = cv2.equalizeHist(gray)

        h, w = gray.shape[:2]
        scale = min(1.0, self.detect_width / max(1, w))
        if scale < 0.999:
            small = cv2.resize(gray, (int(round(w * scale)), int(round(h * scale))), interpolation=cv2.INTER_LINEAR)
        else:
            small = gray
            scale = 1.0

        min_face = max(36, int(round(0.10 * min(small.shape[:2]))))
        ...
        for (x, y, fw, fh) in faces:
            out.append((int(round(x * inv)), int(round(y * inv)), int(round(fw * inv)), int(round(fh * inv))))
        return out
    ...
    def _choose_best_face(
        ...
        best_score = -1e18
        for face in faces:
            iou = rect_iou(face, hint)
            dist2 = rect_center_distance2(face, hint)
            area = float(face[2] * face[3])
            score = 6.0 * iou + 0.00005 * area - 0.00001 * dist2
            if score > best_score:
                best_score = score
                best = face
        return best

    def _init_tracker(self, frame_bgr8: np.ndarray, box: Tuple[int, int, int, int]) -> None:
        tracker, kind = create_opencv_tracker()
        self.tracker = tracker
        self.tracker_kind = kind
        if tracker is None:
            return
        try:
            tracker.init(frame_bgr8, tuple(float(v) for v in box))
        except Exception:
            self.tracker = None
            self.tracker_kind = "None"

    def _update_tracker(self, frame_bgr8: np.ndarray) -> Optional[Tuple[int, int, int, int]]:
        if self.tracker is None:
            return None
        try:
            ok, bbox = self.tracker.update(frame_bgr8)
        except Exception:
            return None
        if not ok:
            return None
        x, y, w, h = [int(round(v)) for v in bbox]
        return clamp_roi((x, y, w, h), frame_bgr8.shape[1], frame_bgr8.shape[0])

    def _compute_roi_rel_face(
        self,
        roi: Tuple[int, int, int, int],
        face: Tuple[int, int, int, int],
    ) -> Tuple[float, float, float, float]:
        rx = (roi[0] - face[0]) / max(1.0, face[2])
        ry = (roi[1] - face[1]) / max(1.0, face[3])
        rw = roi[2] / max(1.0, face[2])
        rh = roi[3] / max(1.0, face[3])
    ...
    def _roi_from_face(
        ...
        rx, ry, rw, rh = rel
        x = int(round(face[0] + rx * face[2]))
        y = int(round(face[1] + ry * face[3]))
        w = int(round(max(12.0, rw * face[2])))
        h = int(round(max(12.0, rh * face[3])))
    ...
    def update(
        ...
        frame8 = convert_to_tracking_bgr8(frame_bgr)
        h, w = frame8.shape[:2]
        manual_roi = clamp_roi(manual_roi, w, h) if manual_roi is not None else None

        if manual_roi is None:
            self.reset()
            return FaceTrackingResult(None, None, False, "Sem ROI")

        if self.pending_roi is not None:
            manual_roi = clamp_roi(self.pending_roi, w, h) or manual_roi
            self.pending_roi = None

        predicted_face = self._update_tracker(frame8)
        if predicted_face is not None:
            self.face_box = predicted_face

        hint = self.face_box or manual_roi
        need_detect = (
            self.force_redetect
            or self.face_box is None
            or predicted_face is None
            or (timestamp_s - self.last_detect_time) >= self.detect_every_s
        )

        if need_detect:
            faces = self._detect_faces(frame8)
            best_face = self._choose_best_face(faces, hint)
            self.last_detect_time = timestamp_s
            self.force_redetect = False
            if best_face is not None:
                self.face_box = best_face
                self._init_tracker(frame8, best_face)
                self.roi_rel_face = self._compute_roi_rel_face(manual_roi, best_face)
            elif predicted_face is None:
                self.face_box = None

        tracked_roi = manual_roi
        mode = "ROI manual"

        if self.face_box is not None and self.roi_rel_face is not None:
            roi_face = self._roi_from_face(self.face_box, self.roi_rel_face, w, h)
            if roi_face is not None:
                tracked_roi = roi_face
                mode = f"Face + {self.tracker_kind}"
        elif predicted_face is not None:
            tracked_roi = manual_roi
            mode = f"Tracker {self.tracker_kind}"

        self.tracked_roi = tracked_roi
        return FaceTrackingResult(
            tracked_roi=tracked_roi,
            face_box=self.face_box,
            tracking_ok=self.face_box is not None or predicted_face is not None,
            mode=mode,
        )
...
class CaptureWorker(QtCore.QObject):
    frameReady = QtCore.Signal(object, float, tuple, object, object, object, str)
    ...
    def __init__(
        ...
        self.target_fps = float(target_fps)
        self.max_display_height = int(max_display_height)
        self.face_tracking_enabled = bool(face_tracking_enabled)
        self.face_detect_every_s = float(face_detect_every_s)
        self._running = False
        ...
        self._face_tracker = FaceTracker(detect_every_s=self.face_detect_every_s) if self.face_tracking_enabled else None
    ...
    def _get_roi_state(self) -> Tuple[Optional[Tuple[int, int, int, int]], bool]:
        with self._roi_lock:
            roi = None if self._roi is None else tuple(self._roi)
            dirty = bool(self._roi_dirty)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            while self._running:
                ...
                roi, roi_dirty = self._get_roi_state()
                tracked_roi = roi
                face_box = None
                tracking_mode = "Sem tracking"

                if self._face_tracker is not None:
                    if roi_dirty:
                        self._face_tracker.request_roi(roi)
                    tracking = self._face_tracker.update(packet.analysis_bgr, packet.timestamp_s, roi)
                    tracked_roi = tracking.tracked_roi
                    face_box = tracking.face_box
                    tracking_mode = tracking.mode

                mean_rgb = None
                if tracked_roi is not None:
                    w_src, h_src = packet.source_size
                    roi_ok = clamp_roi(tracked_roi, w_src, h_src)
...
class VideoWidget(QtWidgets.QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self._tracking_mode: str = "Sem tracking"
    ...
    def paintEvent(self, event: QtGui.QPaintEvent) -> None:
        ...
        if self._face_box is not None and self._source_size[0] > 0:
            face_rect = self._source_to_widget_rect(self._face_box)
            if not face_rect.isNull():
                painter.setPen(QtGui.QPen(QtGui.QColor("#4cc9f0"), 2, QtCore.Qt.PenStyle.DashLine))
                painter.setBrush(QtCore.Qt.BrushStyle.NoBrush)
                painter.drawRoundedRect(face_rect, 6, 6)
                label = f"Rosto rastreado | {self._tracking_mode}"
                fm = painter.fontMetrics()
                pad = 6
                text_w = fm.horizontalAdvance(label) + 2 * pad
                text_h = fm.height() + 2 * pad
                text_rect = QtCore.QRectF(face_rect.left(), max(0.0, face_rect.top() - text_h - 4), text_w, text_h)
                painter.fillRect(text_rect, QtGui.QColor(76, 201, 240, 150))
        ...
        if roi_to_draw is not None and self._source_size[0] > 0:
            ...
            if not rect.isNull():
                ...
                label = f"ROI rPPG: x={roi_to_draw[0]} y={roi_to_draw[1]} w={roi_to_draw[2]} h={roi_to_draw[3]}"
                fm = painter.fontMetrics()
                pad = 6
                text_w = fm.horizontalAdvance(label) + 2 * pad
                text_h = fm.height() + 2 * pad
                text_rect = QtCore.QRectF(rect.left(), min(self.height() - text_h, rect.bottom() + 4), text_w, text_h)
                painter.fillRect(text_rect, QtGui.QColor(0, 208, 132, 170))
                painter.setPen(QtGui.QPen(QtGui.QColor("black"), 1))
                painter.drawText(text_rect.adjusted(pad, 0, -pad, 0), QtCore.Qt.AlignmentFlag.AlignVCenter, label)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("rPPG ao vivo - POS + FFT + Face Tracking")
    ...
    def _build_controls_card(self) -> QtWidgets.QWidget:
        ...
        self.face_tracking_check = QtWidgets.QCheckBox("Ativar tracking facial")
        self.face_tracking_check.setChecked(DEFAULT_FACE_TRACKING)

        self.face_detect_interval_spin = QtWidgets.QDoubleSpinBox()
        self.face_detect_interval_spin.setRange(0.2, 5.0)
        self.face_detect_interval_spin.setDecimals(1)
        self.face_detect_interval_spin.setSingleStep(0.1)
        self.face_detect_interval_spin.setValue(DEFAULT_FACE_DETECT_EVERY_S)
        ...
        layout.addRow(self.face_tracking_check)
        layout.addRow("Redetectar face a cada (s)", self.face_detect_interval_spin)
        ...
        help_label = QtWidgets.QLabel(
            ...
            "Com tracking facial ativado, a ROI passa a acompanhar a face detectada."
        )
    ...
    def _build_metrics_card(self) -> QtWidgets.QWidget:
        ...
        self.tracking_value = QtWidgets.QLabel("--")
        self.tracking_value.setProperty("role", "metric")
    ...
    def start_capture(self) -> None:
        ...
        face_tracking_enabled = bool(self.face_tracking_check.isChecked())
        face_detect_every_s = float(self.face_detect_interval_spin.value())
        ...
        self.worker = CaptureWorker(
            ...
            face_detect_every_s=face_detect_every_s,
        )
    ...
    @QtCore.Slot(object, float, tuple, object, object, object, str)
    def on_frame_ready(
        self,
        display_bgr8: np.ndarray,
        timestamp_s: float,
        ...
        self.video_widget.set_tracking_overlay(
            tracked_roi if isinstance(tracked_roi, tuple) else None,
            face_box if isinstance(face_box, tuple) else None,
            tracking_mode,
        )
        self.latest_frame_time = timestamp_s
        self.tracking_value.setText(tracking_mode)
